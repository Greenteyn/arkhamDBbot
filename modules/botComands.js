...
const {searchCardByName, getFaq, formatCardText} = require("./utils");
const cardsListRu = require("../cardsListRu");
...
const search = async (ctx) => {
    ...
    if (searchQuery.length <= 3) return ctx.reply('Поисковый запрос должен быть длиннее 3 символов');
    foundCards = searchCardByName(searchQuery, cardsListRu);

    if (foundCards.length === 0) return ctx.reply('Ничего не найдено. Повторите поиск.');
    ...
    } else return ctx.reply(`Найдено слишком много совпадений (${foundCards.length}). Задайте более точные параметры поиска.`);
}
...
const callback_query = async (ctx) => {
    ...
    switch (true) {
        case regexSearch.test(callback): {
            ...
            const card = (foundCards) ? foundCards.find(card => card.code === cardId) : cardsListRu.find(card => card.code === cardId);
            const cardText = formatCardText(card.text, card.spoiler);
            const buttons = [Markup.button.callback('❓ FAQ', `faq_${card.code}`)];

            if (card.backimagesrc) buttons.push(Markup.button.callback('↪ Перевернуть', `flip_${card.code}`));

            await ctx.replyWithPhoto({url: `https://ru.arkhamdb.com${card.imagesrc}`}, {
        ...
        case regexFlip.test(callback): {
            ...
            await ctx.replyWithPhoto({url: `https://ru.arkhamdb.com${card.backimagesrc}`}, {
                caption: `<b>${card.name}</b>${cardText}`,
                parse_mode: 'HTML',
                ...Markup.inlineKeyboard([
